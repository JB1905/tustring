...
type SiteSiteMetadataSocialFilterInput = {
  ...
  readonly portfolio: Maybe<StringQueryOperatorInput>;
};
...
type BlogPostFragment = { readonly edges: ReadonlyArray<{ readonly node: (
      Pick<MarkdownRemark, 'excerpt'>
      & { readonly fields: Maybe<(
        Pick<MarkdownRemarkFields, 'slug'>
        & { readonly readingTime: Maybe<Pick<MarkdownRemarkFieldsReadingTime, 'minutes'>> }
      )>, readonly frontmatter: Maybe<Pick<MarkdownRemarkFrontmatter, 'date' | 'title'>> }
    ) }> };

type BlogQueryVariables = Exact<{ [key: string]: never; }>;


type BlogQuery = { readonly allMarkdownRemark: BlogPostFragment };

type BioQueryVariables = Exact<{ [key: string]: never; }>;


type BioQuery = { readonly avatar: Maybe<{ readonly childImageSharp: Maybe<Pick<ImageSharp, 'gatsbyImageData'>> }>, readonly site: Maybe<{ readonly siteMetadata: Maybe<{ readonly author: Maybe<Pick<SiteSiteMetadataAuthor, 'name' | 'summary'>>, readonly social: Maybe<Pick<SiteSiteMetadataSocial, 'linkedin' | 'github' | 'twitter' | 'facebook' | 'portfolio'>> }> }> };
